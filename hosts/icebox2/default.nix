...
By accessing this system, you agree that your actions may be monitored if unauthorized usage is suspected.
  ...
  services = {
    cron = {
      enable = true;
      systemCronJobs = [
        "@reboot root ${pkgs.curl}/bin/curl -fsS -m 10 --retry 5 -o /dev/null https://hc-ping.com/$(< ${config.sops.secrets.healthcheck_icebox2_uptime.path})"
        "*/15 * * * * root ${pkgs.curl}/bin/curl -fsS -m 10 --retry 5 -o /dev/null https://hc-ping.com/$(< ${config.sops.secrets.healthcheck_icebox2_uptime.path})"
      ];
    };
    fail2ban = {
      enable = true;
